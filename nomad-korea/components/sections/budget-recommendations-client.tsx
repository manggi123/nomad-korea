...
interface BudgetRecommendationsClientProps {
  cities: City[];
}

export default function BudgetRecommendationsClient({ cities }: BudgetRecommendationsClientProps) {
  const { filters, hasActiveFilters } = useHomeFilters();

  // 필터 적용
  const allFilteredCities = useMemo(() => {
    return hasActiveFilters ? filterCities(cities, filters) : cities;
            ...
            >
              ...
              <CardContent className="space-y-4">
                ...
                <Button
                  ...
                  asChild
                >
                  <Link href="/search">더 많은 도시 보기</Link>
                </Button>
              </CardContent>
            </Card>
          ))}
